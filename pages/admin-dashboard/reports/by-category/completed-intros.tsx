...
export default function Page() {
  ...
  const courseQ = useQuery({
    queryKey: ["courseAdminDashReports-3"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("course")
        .select(`requestDate, topic(category) `)
  ...
  return (
    <>
      <h1 className="H1">
        Completed Intros <span className="fw-200">by Category</span>
      </h1>
      {renderNoData(courseQ) ?? renderNoData(categoryQ) ?? (
        <TableR pageId={pageId}>
          {months.map((month) => {
            const row = { month: month.yearMonth };
            for (const cat of categoryQ.data) {
              // row[cat]
              const sameCat = courseQ.data.filter((c) => c.topic.category.toLowerCase() === cat.toLowerCase());
              const arr = sameCat.filter((c) => month.includes(c.requestDate));
              row[cat] = arr.length || 0;
            }
            return row;
          })}
        </TableR>
