...
export default function Page() {
  ...
  const courseCategoryQ = useQuery({
    queryKey: ["courseAdminDashReports-2"],
    queryFn: async () => {
      const { data, error } = await supabase.from("course").select(`requestDate, topic(*)`);
      if (error) throw error;
      return data;
    },
  });
  ...
  return (
    <>
      <h1 className="H1">
        New Courses <span className="fw-200">by Category</span>
      </h1>

      {renderNoData(courseCategoryQ) ?? renderNoData(categoryQ) ?? (
        <TableR pageId={pageId}>
          {months.map((month) => {
            const row = { month: month.yearMonth };
            for (const cat of categoryQ.data) {
              row[cat] = courseCategoryQ.data.filter(
                (cc) => month.includes(cc.requestDate) && cc.topic.category === cat
              ).length;
            }
            return row;
          })}
