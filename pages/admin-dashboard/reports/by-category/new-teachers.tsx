...
export default function Page() {
  ...
  const expertiseQ = useQuery({
    queryKey: ["expertiseAdminDashReports-1"],
    queryFn: async () => {
      const { data, error } = await supabase.from("expertise").select(`startDate , topic (category) , teacher (id)`);
      if (error) throw error;
      return data;
    },
  });
  ...
  return (
    <>
      <h1 className="H1">
        New Teachers <span className="fw-200">by Category</span>
      </h1>
      {renderNoData(expertiseQ) ?? renderNoData(categoryQ) ?? (
        <TableR pageId={pageId}>
          {months.map((month) => {
            const row = { month: month.yearMonth };
            for (const cat of categoryQ.data) {
              row[cat] = uniquesCount(
                expertiseQ.data
                  .filter((e) => month.includes(e.startDate) && e.topic.category === cat)
                  .map((e) => e.teacher.id)
              );
            }
            return row;
