...
const Page: NextPage = () => {
  ...
  const recordQ = useQuery({
    queryKey: ["feedbackMasterData-1", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("feedback").select("*").match({ id }).single();
  ...
  const handleDelete = async () => {
    ...
    if (error) {
      ...
      queryClient.invalidateQueries({ queryKey: ["feedbackMasterData-1", id], refetchType: "none" });
      queryClient.invalidateQueries({ queryKey: ["feedbackMasterData-1"] });
    ...
    renderNoData(recordQ) ?? (
      <div className="space-y-8">
        ...
        {recordQ.data ? (
          ...
          <div>
            There is no record with <code>id: {id}</code> in feedback.
          </div>
        )}
      </div>
    )
