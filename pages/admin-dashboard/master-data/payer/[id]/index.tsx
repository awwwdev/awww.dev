...
const Page: NextPage = () => {
  ...
  const recordQ = useQuery({
    queryKey: ["payerMasterData-1", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("payer").select("*").match({ id }).single();
  ...
  const handleDelete = async () => {
    ...
    if (error) {
      ...
      queryClient.invalidateQueries({ queryKey: ["payerMasterData-1", id], refetchType: "none" });
      queryClient.invalidateQueries({ queryKey: ["payerMasterData-1"] });
        ...
        {recordQ.data ? (
          <>
            <div className="flex gap-4 justify-end p-4 b-1 b-gray5 rd-lg bg-gray2">
              <Link href={`/admin-dashboard/master-data/payer/${id}/edit`} className="bf-i-ph-note-pencil btn-ghost">
                Edit
              </Link>
          ...
          <div>
            There is no record with <code>id: {id}</code> in payer.
          </div>
        )}
      </div>
    )
