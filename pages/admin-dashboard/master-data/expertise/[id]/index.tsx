...
const Page: NextPage = () => {
  ...
  const recordQ = useQuery({
    queryKey: ["expertiseMasterData-1", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("expertise").select("*").match({ id }).single();
  ...
  const handleDelete = async () => {
    ...
    if (error) {
      ...
      queryClient.invalidateQueries({ queryKey: ["expertiseMasterData-1", id], refetchType: "none" });
      queryClient.invalidateQueries({ queryKey: ["expertiseMasterData-1"] });
        ...
        {recordQ.data ? (
          <>
            <div className="flex gap-4 justify-end p-4 b-1 b-gray5 rd-lg bg-gray2">
              <Link
                href={`/admin-dashboard/master-data/expertise/${id}/edit`}
          ...
          <div>
            There is no record with <code>id: {id}</code> in expertise.
          </div>
        )}
      </div>
    )
        ...
        >
          ...
          <AlertDialog.Description className="mt-2 text-sm font-normal text-gray-700 dark:text-gray-400">
            ...
            <div className="info-line text-xs">
              If there is any other record with reference (Foreign Key) to this record, you need to delete those first.
