...
const Page: NextPage = () => {
  ...
  const recordQ = useQuery({
    queryKey: ["currencyExchangeRateMasterData-1", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("currencyExchangeRate").select("*").match({ id }).single();
  ...
  const handleDelete = async () => {
    const { error } = await supabase.from("currencyExchangeRate").delete().eq("id", id);
    if (error) {
      toast.error("Failed to delete the record. | Error: " + error.message);
    } else {
      toast.success("Record is successfully deleted.");
      queryClient.invalidateQueries({ queryKey: ["currencyExchangeRateMasterData-1", id], refetchType: "none" });
      queryClient.invalidateQueries({ queryKey: ["currencyExchangeRateMasterData-1"] });
        ...
        {recordQ.data ? (
          <>
            <div className="flex gap-4 justify-end p-4 b-1 b-gray5 rd-lg bg-gray2">
              <Link
                href={`/admin-dashboard/master-data/currency-exchange-rate/${id}/edit`}
          ...
          <div>
            There is no record with <code>id: {id}</code> in currency exchange rate.
          </div>
        )}
      </div>
    )
...
export async function getServerSideProps(ctx) {
  ...
  await queryClient.prefetchQuery({
    queryKey: ["currencyExchangeRateMasterData-11", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("currencyExchangeRate").select("*").match({ id }).single();
      if (error) throw error;
      return data;
