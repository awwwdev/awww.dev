...
const Page: NextPage = () => {
  ...
  const recordQ = useQuery({
    queryKey: ["paymentByPayerMasterData-1", id],
    queryFn: async () => {
      const { data, error } = await supabase.from("paymentByPayer").select("*").match({ id }).single();
  ...
  const handleDelete = async () => {
    const { error } = await supabase.from("paymentByPayer").delete().eq("id", id);
    if (error) {
      toast.error("Failed to delete the record. | Error: " + error.message);
    } else {
      toast.success("Record is successfully deleted.");
      queryClient.invalidateQueries({ queryKey: ["paymentByPayerMasterData-1", id], refetchType: "none" });
      queryClient.invalidateQueries({ queryKey: ["paymentByPayerMasterData-1"] });
        ...
        {recordQ.data ? (
          <>
            <div className="flex gap-4 justify-end p-4 b-1 b-gray5 rd-lg bg-gray2">
              <Link
                href={`/admin-dashboard/master-data/payment-by-payer/${id}/edit`}
          ...
          <div>
            There is no record with <code>id: {id}</code> in payment by payer.
          </div>
        )}
      </div>
    )
