...
const Inbox = () => {
  ...
  const handleArchive = async (mailId) => {
    const { data, error } = await supabase.from("dashboardGeneralMailLog").update({ isArchived: true }).eq("id", mailId);

    if (error) {
      toast.error("Error while marking message as read");
    } else {
      toast.success("Message marked as read successfully!");
      queryClient.invalidateQueries(["dashboardGeneralPayerDash-1"]);
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-center">
        ...
        <Link href="/payer-dashboard/inbox/end-of-package/archived" className="btn-ghost-accent">
          Read messages
        </Link>
      </div>
          ...
          return (
            <details className="border border-gray-400 rounded-t-xl rounded-br-xl bg-gray-100" key={mail.id}>
              <summary className="flex justify-between p-4 cursor-pointer rounded-t-xl rounded-br-xl hover:bg-gray-300">
                <p className="inline text-sm">End of Package</p>
